 ...
 */
const SIGNALS: Record<SubstantiveClauseType, Signal> = {
  pricing_terms: {
    keywords: [
      /\bVWAP\b/i,
...
export function classifyDominant(text: string): SubstantiveClauseType | null {
  const scores = new Map<SubstantiveClauseType, number>();

  for (const type of Object.keys(SIGNALS) as SubstantiveClauseType[]) {
    const sig = SIGNALS[type];
    let score = 0;
    for (const rx of sig.keywords) {
      if (rx.test(text)) score += sig.boost;
    }
    scores.set(type, score);
  }

  let bestType: SubstantiveClauseType | null = null;
  let bestScore = 0;
  ...
  if (bestScore === 0 || bestType === null) return null;

  const runnersUp = [...scores.entries()].filter(
    ([t, s]) => t !== bestType && s === bestScore,
  );
...
export function classificationStrength(text: string): number {
  ...
  const kwHits = SIGNALS[dominant].keywords.filter((rx) =>
    rx.test(text),
  ).length;
  ...
  return Math.min(
    1,
    normalizedSeparation * 0.6 + normalizedHits * 0.4,
  );
}
