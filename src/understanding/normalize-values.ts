...
export function parseUsdScalar(raw: string): number | null {
...
export function extractUsdAmounts(text: string): number[] {
  const found: number[] = [];
...
export function parsePercentLiteral(text: string): number | null {
...
export function extractPercentValues(text: string): number[] {
  ...
  for (const m of text.matchAll(/\b(\d{1,2}(?:\.\d+)?)\s*%/g)) {
    const n = Number.parseFloat(m[1]);
    if (Number.isFinite(n)) out.push(Math.round(n * 1e4) / 1e4);
  }
  return [...new Set(out)];
}
