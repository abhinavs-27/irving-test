 ...
 */
function splitDenseSingleBlock(text: string, targetLen: number): string[] {
 ...
 */
function mergeShortFragments(parts: string[], minLen: number): string[] {
  const merged: string[] = [];
 ...
 */
export function splitIntoParagraphs(section: Clause): Clause[] {
  ...
  let pi = 1;
  const paragraphs: Clause[] = atomicTexts.map((text) => ({
    id: `${section.id}.p${pi++}`,
    title: '',
    text,
 ...
 */
export function withParagraphChildren(section: Clause): Clause {
  ...
  let next: Clause = { ...section, children };
 ...
 */
export function segmentSectionsIntoParagraphs(sections: Clause[]): Clause[] {
  ...
  for (const s of out) {
    ...
    for (const p of s.children) totalParaChars += p.text.length;
  }

  legalDocLog.info('Document paragraph segmentation summary', {
    topLevelSections: out.length,
    totalParagraphUnits: totalParas,
    avgParagraphCharsGlobal:
      totalParas > 0 ? Math.round(totalParaChars / totalParas) : 0,
    paragraphsPerSection: Object.fromEntries(
      out.map((s) => [s.id, s.children.length]),
    ),
  });
