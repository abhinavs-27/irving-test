...
import {
  mergeIncompleteParagraphs,
  paragraphEndsIncomplete,
} from './paragraph-integrity.js';

import type { Clause } from '../clause/clause.js';

function para(id: string, text: string): Clause {
  return {
    id,
    title: '',
    text,
...
describe('paragraph integrity', () => {
  ...
  it('merges incomplete fragments and renumbers ids', () => {
    ...
    assert.strictEqual(
      merged[0]!.text,
      'issued to it by the Company pursuant to Section 2.',
    );
