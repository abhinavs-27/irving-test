 ...
 */
export function paragraphEndsIncomplete(text: string): boolean {
  ...
  if (
    lastWord &&
    /^(the|a|an|to|of|by|from|for|with|at|in|on|as|it|or|and|but|if|when|that|which|issued|such|not|no|any|each|both|either)$/i.test(
      lastWord,
    )
  )
    return true;

  if (/\b(?:under|over|upon|into|onto)\s+$/i.test(s)) return true;
 ...
 */
export function mergeIncompleteParagraphs(
  sectionId: string,
  paras: Clause[],
): Clause[] {
  if (paras.length <= 1) return renumberIds(sectionId, paras);
...
function renumberIds(sectionId: string, paras: Clause[]): Clause[] {
