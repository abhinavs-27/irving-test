...
function endsWithIncompleteSentence(s: string): boolean {
  const t = s.trimEnd();
...
function startsLikeContinuation(s: string): boolean {
  const t = s.trimStart();
  if (t.length === 0) return false;
  const c = t[0]!;
 ...
 */
export function mergeSoftParagraphBreaks(raw: string): string {
  const chunks = raw.split(/\n\s*\n+/).map((c) => c.trim()).filter(Boolean);
  if (chunks.length <= 1) return raw.trim();
  ...
  for (const chunk of chunks) {
    ...
    if (
      endsWithIncompleteSentence(prev) &&
