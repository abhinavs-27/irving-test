 ...
 */
function stripRepeatedLinesAcrossPages(
  pageLines: string[][],
  pageRatioThreshold: number,
): string[][] {
  const pageCount = pageLines.length;
  if (pageCount === 0) return pageLines;

  const lineToPages = new Map<string, Set<number>>();

  for (let p = 0; p < pageCount; p++) {
    const seenOnPage = new Set<string>();
    for (const line of pageLines[p]) {
      const normalized = line.trim().replace(/\s+/g, ' ');
      if (normalized.length < MIN_LINE_LENGTH_FOR_REPEAT_DETECTION) continue;
      if (seenOnPage.has(normalized)) continue;
  ...
  const thresholdPages = Math.max(
    2,
    Math.ceil(pageCount * pageRatioThreshold),
  );

  for (const [line, pages] of lineToPages) {
    if (pages.size >= thresholdPages) strip.add(line);
  }
 ...
 */
function repairBrokenWrapping(lines: string[]): string[] {
  const out: string[] = [];
...
function pagesToCleanedText(
  pages: Array<{ num: number; text: string }>,
  repeatedLinePageRatio: number,
): string {
  const pageLineArrays = pages.map((p) =>
    repairBrokenWrapping(splitLines(p.text)),
  );

  const withoutRepeat = stripRepeatedLinesAcrossPages(
    pageLineArrays,
    repeatedLinePageRatio,
  );
 ...
 */
export function createPdfParseLoader(
  options: PdfParseLoaderOptions = {},
): DocumentTextLoader {
  const repeatedLinePageRatio =
    options.repeatedLinePageRatio ?? DEFAULT_HEADER_FOOTER_PAGE_RATIO;
    ...
    async load(path: string): Promise<string> {
      ...
      try {
        ...
        const cleaned = normalizeWhitespace(
          pagesToCleanedText(result.pages, repeatedLinePageRatio),
        );
