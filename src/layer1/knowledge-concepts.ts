...
const CONCEPT_TYPE_AND_DESC: Record<
  string,
  { type: GraphConceptType; description: string; parent?: string }
> = {
  [`${CONCEPT_ID_PREFIX}intraday_purchase`]: {
    type: 'purchase_type',
    description:
      'Trades or purchases made during the intraday window in the defined valuation period',
  ...
  [`${CONCEPT_ID_PREFIX}exchange_cap_issuance`]: {
    type: 'issuance',
    description:
      'Cap on number of shares or value issuable in connection with a listing exchange or similar rule',
...
export function buildConceptRegistry(
  ...
  sourceByConcept: ReadonlyMap<string, readonly string[]>,
): { concepts: Record<string, GraphConceptRecord>; undefinedConceptRefs: string[] } {
  const used = new Set<string>();
  for (const r of relationships) {
    for (const end of [r.source, r.target] as const) {
      if (end.startsWith(CONCEPT_ID_PREFIX)) used.add(end);
    }
  }
  let grow = true;
  while (grow) {
  ...
  const undefinedConceptRefs: string[] = [];
  const concepts: Record<string, GraphConceptRecord> = {};
  ...
  for (const id of used) {
    ...
    const rec: GraphConceptRecord = {
      type: def.type,
      description: def.description,
      source_paragraphs: [...paras].sort(),
    };
    if (def.parent) rec.parent = def.parent;
    concepts[id] = rec;
  }
