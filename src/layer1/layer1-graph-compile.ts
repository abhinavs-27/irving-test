...
function collectReferencedOrgIds(
  relationships: DocumentRelationship[],
  events: GraphEvent[],
): Set<string> {
  const s = new Set<string>();
  for (const r of relationships) {
    if (r.target.startsWith('org:')) s.add(r.target);
  }
...
function firstStructuralBlockId(
 ...
 */
export function normalizeLayer1Graph(
  ...
  for (let pass = 0; pass < 8; pass++) {
    ...
    for (const oid of orgIds) {
      if (refd.has(oid)) continue;
      if (!anchor) continue;
      relationships.push({ type: 'defines', source: anchor, target: oid });
      refd.add(oid);
    }

    const execAgreements =
      events.find((e) => e.kind === 'agreement_execution')?.agreement_types ??
      [];
    ...
    const termBlocks = Object.entries(block_registry).filter(
      ([, b]) => b.semantic_role === 'termination',
    );
    for (const [bid] of termBlocks) {
      const evId = `${EVENT_ID_PREFIX}termination_${bid.replace(/[^a-z0-9]+/gi, '_')}`;
      const hasTrig = relationships.some(
        (r) =>
          r.type === 'triggers' && r.source === bid && r.target === evId,
      );
...
function dedupeRelationships(relationships: DocumentRelationship[]): void {
...
export function validateLayer1Graph(
  payload: Layer1GraphPayload,
  sections: Clause[],
): { ok: boolean; issues: { path: string; code: string; message: string }[] } {
  ...
  for (const s of sections) {
    for (const c of s.children) {
      if (c.type === 'paragraph') allParagraphIds.add(c.id);
    }
  }
  const paraToBlock = new Map<string, string>();
  ...
  for (const [, b] of Object.entries(block_registry)) {
    if (b.semantic_role === 'termination') {
      const evId = events.find(
        (e) =>
          e.kind === 'agreement_termination' &&
          e.source_block_ids.includes(b.id),
      )?.id;
      if (!evId) {
        issues.push({
        ...
        const trig = relationships.some(
          (x) => x.type === 'triggers' && x.source === b.id && x.target === evId,
        );
        if (!trig) {
          issues.push({
            path: `block_registry/${b.id}`,
