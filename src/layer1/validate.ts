...
function validateFactsV2(
  ...
  if (Array.isArray(f['percentages'])) {
    for (const p of f['percentages'] as unknown[]) {
  ...
  for (const key of ['dollar_amounts', 'price_thresholds'] as const) {
    const arr = f[key];
  ...
  if (Array.isArray(f['time_windows'])) {
    for (const tw of f['time_windows'] as unknown[]) {
...
function validateEntities(
  path: string,
  e: { organizations?: string[]; people?: string[] },
  issues: Layer1ValidationIssue[],
): void {
  for (const field of ['organizations', 'people'] as const) {
    const a = e[field];
...
function blockAtomicMismatch(
  blockType: Layer1BlockType,
  atomic: AtomicClauseKind,
): boolean {
...
function walk(
  ...
  for (let i = 0; i < clauses.length; i++) {
    ...
    if (c.pricing) validatePricing(path, c.pricing, issues);
    ...
    if (c.signals && typeof c.signals === 'object') {
      ...
      if ('market' in sig || 'security' in sig || 'agreement' in sig) {
        issues.push({
          ...
          message:
            'signals must use market_signals, legal_signals, security_signals only',
        });
      }
    }

    if (c.entities) validateEntities(path, c.entities, issues);
    ...
    if (c.blocks) {
      for (const b of c.blocks) {
        ...
        if ('intent' in b && (b as { intent?: unknown }).intent != null) {
          issues.push({
            path: `${path}/blocks/${b.id}`,
            code: 'intent_forbidden',
            message:
              'structured intent/summary fields are not allowed on Layer 1 blocks',
          });
        }
      }
    ...
    if (c.children.length) walk(c.children, `${path}/children`, issues);
  }
}

export function validateLayer1Tree(clauses: Clause[]): Layer1ValidationResult {
  const issues: Layer1ValidationIssue[] = [];
