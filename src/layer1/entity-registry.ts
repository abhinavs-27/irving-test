...
import { expandLayer1SignalBuckets, type FullSignalShape } from './signal-shape.js';
import type {
  EntityRegistry,
  GraphEntityRecord,
  Layer1ClauseSignals,
...
function entityMergeKey(s: string): string {
  return s
    .toLowerCase()
    .replace(/\b([a-z])\s*\./g, '$1')
    .replace(/&/g, ' and ')
...
function toEntityIdFromName(name: string): string {
  const s = name
    .replace(/[.,&]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
 ...
 */
export function collectEntityNames(sections: Clause[]): string[] {
  const out: string[] = [];
  for (const s of sections) {
    for (const o of s.entities?.organizations ?? []) {
      if (o.trim().length > 1) out.push(o.trim());
    }
    for (const p of s.children) {
      if (p.type !== 'paragraph' || !p.signals?.legal_signals) continue;
      const c = p.signals.legal_signals.counterparty;
      if (c) {
        for (const part of c.split(/\s*\|\s*/)) {
          if (part.trim().length > 1) out.push(part.trim());
        }
      }
    }
  }
 ...
 */
export function buildEntityRegistry(sections: Clause[]): EntityRegistry {
  const byKey = new Map<string, { id: string; record: GraphEntityRecord }>();
...
export const B_RILEY_PRINCIPAL_CANONICAL =
  'B. Riley Principal Capital II, LLC' as const;

export const B_RILEY_PRINCIPAL_ID =
  `${ENTITY_ID_PREFIX}b_riley_principal_capital_ii_llc` as const;

function isRileyMentioned(text: string): boolean {
  return /b\.\s*riley|b\s*riley|principal capital ii|riley principal capital/i.test(
    text,
  );
}
 ...
 */
export function consolidateBRileyPrincipalEntity(
  registry: EntityRegistry,
): { registry: EntityRegistry; idRemap: ReadonlyMap<string, string> } {
  const remapped = new Map<string, string>();
  const toMerge: string[] = [];
  for (const [id, r] of Object.entries(registry)) {
    if (
      isRileyMentioned(r.canonical_name) ||
  ...
  const aliases = new Set<string>(['B. Riley', 'B Riley']);
  for (const id of toMerge) {
    const r = registry[id]!;
  ...
  const out: EntityRegistry = {};
  for (const [id, r] of Object.entries(registry)) {
    if (toMerge.includes(id)) continue;
    out[id] = r;
  }
  out[B_RILEY_PRINCIPAL_ID] = {
    ...
    aliases: [...aliases]
      .filter(
        (a) =>
          a && a.toLowerCase() !== B_RILEY_PRINCIPAL_CANONICAL.toLowerCase(),
      )
      .sort(),
  };
 ...
 */
export function resolveNameToEntityId(
  ...
  for (const [id, r] of Object.entries(registry)) {
    if (r.canonical_name === t) return id;
    for (const a of r.aliases) {
      if (a === t) return id;
    }
  }
 ...
 */
export function resolveIssuerEntityId(
  sections: Clause[],
  registry: EntityRegistry,
): string | undefined {
  ...
  for (const s of [...sections].sort((a, b) => a.id.localeCompare(b.id))) {
    for (const o of s.entities?.organizations ?? []) {
      if (o.trim()) orgNames.push(o.trim());
    }
  }
  ...
  for (const id of sortedIds) {
    if (id === B_RILEY_PRINCIPAL_ID) continue;
    const r = registry[id]!;
 ...
 */
export function applyEntityIdsToParagraphs(
  sections: Clause[],
  registry: EntityRegistry,
): Clause[] {
  return sections.map((s) => {
    ...
    const children = s.children.map((p) => {
      ...
      return {
        ...p,
        signals: buildSignalsWithEntityRegistry(p.text, registry) as unknown as Layer1ClauseSignals,
      };
    });
    return { ...s, children };
  });
 ...
 */
export function buildSignalsWithEntityRegistry(
  text: string,
  registry: EntityRegistry,
): FullSignalShape {
