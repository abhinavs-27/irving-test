...
import type { SectionEntities } from './types.js';
...
function dedupeNormalized(list: string[]): string[] {
  ...
  for (const raw of list) {
    ...
    if (/^(the|a|an)\s+company$/i.test(n)) continue;
...
const ORG_RE =
  /\b([A-Z0-9][A-Za-z0-9&.'\u00ae-]*(?:\s+[A-Z0-9][A-Za-z0-9&.'-]*)*)\s*,?\s*(?:LLC|L\.L\.C\.|Inc\.?|Corp\.?|Ltd\.?|L\.P\.?|LP)\b/gi;

const NAMED =
  /\b(Gelesis(?:\s+Holdings,?\s+Inc\.?)?|B\.\s*Riley Principal Capital II,?\s+LLC?|B\.\s*Riley)(?![A-Za-z])/gi;

/**
 * Registrant + named orgs + counterparty(s) from signals; deduped and normalized.
 */
export function buildSectionEntities(
  children: Clause[],
  registrantName: string | null,
): SectionEntities {
  const organizations: string[] = [];
  if (registrantName) organizations.push(registrantName);

  const all = children.map((c) => c.text).join('\n');

  for (const m of all.matchAll(NAMED)) {
    if (m[1]) organizations.push(m[1]!.replace(/\s+/g, ' ').trim());
  }
  for (const m of all.matchAll(ORG_RE)) {
    if (m[0] && m[0].length > 3 && m[0].length < 90) {
      const o = m[0].replace(/\s+/g, ' ').trim();
      if (!/^(with|by and|from)\b/i.test(o)) organizations.push(o);
    }
  }
  ...
  for (const p of children) {
    ...
    if (cp) {
      for (const part of cp.split(/\s*\|\s*/)) {
        if (part.trim().length > 2) organizations.push(part.trim());
      }
    }
  }
  ...
  for (const m of all.matchAll(
    ...
    if (line) people.push(line.replace(/\s+/g, ' ').trim());
  }
  for (const m of all.matchAll(
    /\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}),\s+(?:Chief|President|Principal|Senior|CFO|CEO|COO|Secretary|Treasurer|Officer|Director|Accounting)\b/gi,
  )) {
    if (m[1]) people.push(m[1].replace(/\s+/g, ' ').trim());
  }
