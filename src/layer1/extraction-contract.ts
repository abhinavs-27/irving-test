...
function collectParagraphIds(sections: Clause[]): Set<string> {
  const s = new Set<string>();
  const walk = (nodes: Clause[]): void => {
    for (const n of nodes) {
      if (n.type === 'paragraph') s.add(n.id);
...
function paragraphById(sections: Clause[]): Map<string, Clause> {
...
export function validateExtractionContract(
  ...
  const assignedPara = new Set<string>();
  ...
  for (const e of events) {
    ...
    const kind = e.kind;
    const hasDateInSource =
      kind === 'agreement_execution' || kind === 'agreement_termination';
    if (hasDateInSource) {
      const paras = e.source_block_ids.flatMap((bid) =>
        block_registry[bid]?.paragraph_ids ?? [],
      );
      let textHasIso = false;
      for (const pid of paras) {
        const px = paraMap.get(pid);
        if (px?.facts?.dates?.length) textHasIso = true;
      }
    ...
    if (e.kind === 'agreement_termination') {
      ...
      if (
        ...
        violations.push({
          ...
          path: ep,
          message:
            'termination event must include substantive conditions text (not generic label only)',
        });
      }
    }
  }

  const governPairs = new Set<string>();
  const triggerPairs = new Set<string>();
  const constrainPairs = new Set<string>();
  ...
  for (const [bid, br] of Object.entries(block_registry)) {
    if (br.semantic_role !== 'termination') continue;
  ...
  for (const [bid, br] of Object.entries(block_registry)) {
    if (br.semantic_role !== 'constraint') continue;
    const issuer = issuerResolved ?? events.find((x) => x.primary_entity_id)?.primary_entity_id;
    const cp =
      events.find((x) => x.counterparty_entity_ids?.length)?.counterparty_entity_ids?.[0];
  ...
  for (const s of sections) {
    if (s.type !== 'section' || !s.blocks?.length) continue;
    const constraints = buildSectionConstraints(s);
    const byPid = new Map<string, SectionConstraintEntry[]>();
