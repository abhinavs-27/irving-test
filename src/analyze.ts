...
import {
  applyExtractionContractFixes,
} from './layer1/extraction-contract-fix.js';
import {
  validateExtractionContract,
} from './layer1/extraction-contract.js';
...
function buildPricingMapForSections(sections: Clause[]): Map<string, BlockPricingModel> {
  const m = new Map<string, BlockPricingModel>();
  for (const s of sections) {
    if (s.type !== 'section') continue;
    const paraBy = new Map(
      s.children
        .filter((x) => x.type === 'paragraph')
        .map((p) => [p.id, p] as [string, Clause]),
    );
...
function clauseTreeForJson(clauses: Clause[]): unknown[] {
  return clauses.map((c) => {
    ...
    if (c.type === 'paragraph' && c.signals) o.signals = c.signals;
    if (c.type === 'paragraph' && c.facts) o.facts = c.facts;
    if (c.type === 'section' && c.entities) o.entities = c.entities;
    if (c.type === 'section') {
      const ids = (c.blocks ?? []).map((b) => b.id);
      if (ids.length) o.block_ids = ids;
...
`);
  ...
  if (outText) {
    ...
    console.error(
      `[analyze] wrote full text (${text.length} chars) → ${absolute}`,
    );
  }

  const sections0 = normalizeParagraphNodesAndGroupBlocks(
  ...
  let contractViolations = validateExtractionContract(
    sections,
    graphPayload.entity_registry,
    graphPayload.block_registry,
    graphPayload.events,
    graphPayload.relationships,
  );
  for (let round = 0; round < 12 && contractViolations.length > 0; round++) {
    applyExtractionContractFixes(sections, graphPayload);
    contractViolations = validateExtractionContract(
      sections,
      graphPayload.entity_registry,
      graphPayload.block_registry,
      graphPayload.events,
  ...
  const layer2_validation = validateLayer2Tree(
    sections,
    relationships,
    entity_registry,
    eventIdSet,
  ...
  if (!layer1_validation.ok) {
    console.error(
      `[analyze] Layer 1 validation issues (${layer1_validation.issues.length}):`,
    );
    ...
    if (layer1_validation.issues.length > 20) {
      console.error(
        `  … and ${layer1_validation.issues.length - 20} more (fix schema pipeline)`,
      );
    }
  }
    ...
    if (!extraction_contract_ok) {
      console.error(
        `[analyze] Extraction contract FAILED — ${contractViolations.length} rule(s) still violated after auto-fix.`,
      );
    ...
    console.error(
      `[analyze] Layer 2 validation issues (${layer2_validation.issues.length}):`,
    );
  ...
  if (
    ...
    if (!layer1_graph_validation.ok) {
      console.error(
        `[analyze] Layer 1 graph invariant issues (${layer1_graph_validation.issues.length}):`,
      );
  ...
  if (printFlat) {
    ...
    for (const c of sections) {
      console.log(
        `${c.type}\t${c.id}\ttitle=${JSON.stringify(c.title)}\ttextChars=${c.text.length}`,
      );
    }
  }
}
