...
const SEC_FOOTER_LINE = /^(SIGNATURES?|Pursuant to the requirements of the Securities Exchange Act)/i;
function emptyClause(id, title, text, type) {
    return { id, title, text, type, children: [] };
}
/** Pass 1: Item headers only (titles from regex capture only). */
function detectSecItems(text) {
    const matches = [];
 ...
 */
function findFooterStart(text, minStart) {
    let last = null;
    for (const { lineStart, line } of eachPhysicalLine(text)) {
        if (lineStart < minStart)
            continue;
        const trimmed = line.replace(/\r$/, '').trim();
        if (!trimmed)
            continue;
        if (SEC_FOOTER_LINE.test(trimmed))
            last = lineStart;
    }
    return last;
}
...
export function extractSections(text) {
    ...
    if (dupIds.length > 0) {
        legalDocLog.warn('Multiple line breaks for same Item number (e.g. TOC + body); keeping each segment', { duplicateItemNumbers: dupIds.map(([id, n]) => ({ id, segments: n })) });
    }
    const lastItemStart = items.length > 0 ? items[items.length - 1].start : 0;
    ...
    if (items.length > 0) {
        legalDocLog.info(`Detected sections:\n${items
            .map((m) => `  • ${m.id} ${m.title}`.trimEnd())
            .join('\n')}`);
    }
    const sections = [];
    const firstItemStart = items[0]?.start ?? text.length;
    const headerEnd = Math.min(firstItemStart, footerStart ?? text.length);
    ...
    if (footerStart !== null) {
        ...
        if (footerText.length > 0) {
            sections.push(emptyClause('signature', '', footerText, 'footer'));
        }
    }
...
export function parseSectionInnerClauses(section) {
    return splitIntoParagraphs(section);
}
//# sourceMappingURL=extract-sections.js.map