...
export function validateFilingSchemaShape(filing) {
    const issues = [];
...
function collectParagraphIdsFromSections(nodes) {
    const ids = new Set();
    const duplicates = [];
    const walk = (n) => {
...
function walkParagraphNodes(nodes, fn) {
    if (!nodes)
        return;
    for (const n of nodes) {
        if (n.type === 'paragraph')
        ...
        if (n.children?.length)
            walkParagraphNodes(n.children, fn);
    }
}
/** Business rules beyond JSON Schema (graph invariants). */
export function validateFilingRules(filing) {
    const issues = [];
    ...
    if (filing.sections?.length) {
        const { ids: paraInTree, duplicates } = collectParagraphIdsFromSections(filing.sections);
    ...
    for (const [bid, b] of Object.entries(block_registry)) {
        const br = b;
        for (const pid of br.paragraph_ids) {
            const hasEdge = relationships.some((r) => r.type === 'governs' && r.source === bid && r.target === pid);
            if (!hasEdge) {
                issues.push({
    ...
    for (const [bid, b] of Object.entries(block_registry)) {
        ...
        if (br.type === 'constraint') {
            ...
            for (const pid of br.paragraph_ids) {
                ...
                const facts = para?.facts;
                if (facts) {
                    if (facts.percentages?.length)
                        numeric = true;
                    if (facts.dollar_amounts?.some(isUsd))
                        numeric = true;
            ...
            if (!numeric) {
                issues.push({
                    ...
                    path: `block_registry/${bid}`,
                    message: 'constraint block must include at least one numeric fact (%, USD, or shares) on a paragraph',
                });
            }
        }
    }
    const semanticByPara = new Map();
    for (const [bid, b] of Object.entries(block_registry)) {
        const br = b;
...
function findParagraph(nodes, id) {
    if (!nodes)
        return undefined;
    for (const n of nodes) {
...
export function validateFiling(filing) {
    ...
    const rules = validateFilingRules(filing);
    return {
        ok: rules.ok,
...
//# sourceMappingURL=filing-validate.js.map