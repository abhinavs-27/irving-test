...
function modeFromParagraph(text) {
    const t = text;
    const hasIntraday = /\bintraday\b/i.test(t);
    const hasRegular = /\bregular\s+Purchase\b/i.test(t) ||
        (!hasIntraday && /\bVWAP\b/i.test(t) && /full|primary|session/i.test(t));
    if (hasIntraday) {
        const multi = /\b(?:two|2|each|per)\s+.*\bsegment/i.test(t) ||
 ...
 */
export function buildPricingModelForBlock(_sectionId, block, paragraphById) {
    if (block.type !== 'pricing_mechanism')
        return undefined;
    ...
    for (const p of paras) {
        ...
        if (/\bopen(?:ing)?\b.*\bclose|exclude.*open|exclude.*close|first.*last.*(?:minute|min)/i.test(t)) {
        ...
        if (!byKey.has(key)) {
            ...
            if (mode.discount_percent != null)
                ex.discount_percent = mode.discount_percent;
            if (mode.volume_threshold_truncation) {
                ex.volume_threshold_truncation = true;
            }
            if (mode.excludes_open_close)
                ex.excludes_open_close = true;
            if (mode.multi_window)
                ex.multi_window = true;
        }
    }
    if (byKey.size === 0) {
        const collected = [];
        for (const p of paras) {
            ...
            if (!/\bVWAP\b/i.test(t))
                continue;
            const pr = extractExplicitClausePricing(t);
            const w = /\bintraday\b/i.test(t)
                ? 'intraday'
                : 'full_session';
            const name = w === 'intraday'
                ? 'intraday_purchase'
                : 'regular_purchase';
            const m = {
                name,
        ...
        if (collected.length === 0)
            return undefined;
        return applyBlockDiscount({ type: 'vwap_discount', method: 'VWAP', modes: mergeModes(collected) }, paras);
    }
...
function applyBlockDiscount(model, paras) {
...
export function buildBlockPricingModelMap(sectionId, section, paragraphById) {
    const map = new Map();
