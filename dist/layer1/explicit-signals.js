...
function extractExchangeLiteral(text) {
...
function extractSecurityTypeLiteral(text) {
    const lower = text.toLowerCase();
    if (/\bpre[- ]funded\s+warrants?\b/i.test(lower))
        return 'pre_funded_warrants';
    if (/\bcommon\s+stock\b/i.test(lower))
        return 'common_stock';
    if (/\bpreferred\s+stock\b/i.test(lower))
        return 'preferred_stock';
    if (/\bwarrants?\b/i.test(lower))
        return 'warrants';
    return null;
}
/**
 ...
 */
function extractAgreementTypeLiteral(text) {
    const quoted = text.match(/"([^"]*(?:Agreement|Indenture|Guarantee)[^"]*)"/i);
    if (quoted?.[1]) {
        const inner = quoted[1].replace(/\s+/g, ' ').trim();
        if (inner.length >= 8 && inner.length <= 200)
            return inner;
    }
    const known = /\b(Common\s+Stock\s+Purchase\s+Agreement|Security\s+Purchase\s+Agreement|Securities\s+Purchase\s+Agreement|Registration\s+Rights\s+Agreement|Equity\s+Distribution\s+Agreement)\b/i.exec(text);
    if (known?.[1])
        return known[1].trim();
    return null;
}
 ...
 */
function extractCounterpartyLiteral(text) {
    const m = text.match(/\bbetween\s+([^,]{2,120}?)\s+and\s+([^,]{2,120}?)(?:\s*[,(]|\s+under|\s+dated|\s+pursuant|\s+entered|\s+effective|\s+of\s+which|$)/i);
    if (m?.[1] && m[2]) {
    ...
    const among = text.match(/\bby\s+and\s+among\s+([^.;\n]{6,200}?)(?:[.;]|\n|$)/i);
    if (among?.[1]) {
        return among[1].replace(/\s+/g, ' ').trim();
    }
    const w = text.match(/(?:^|[.;]\s+)(?:together\s+with|with|from)\s+((?:(?:[A-Z0-9][A-Za-z&.'\u00ae-]*\s+){0,4}(?:LLC|L\.L\.C\.|Inc|Corp|Ltd|L\.P\.|LP)))\b/gi);
    if (w?.[1]) {
 ...
 */
function extractBrRileyOrg(text) {
    const m = text.match(/\bB\.\s*Riley[^.;]{0,100}LLC\b/gi)?.[0] ??
...
function extractLegalRole(text, hasBr) {
    if (hasBr &&
        /\b(?:purchas|Capital|Lender|invest|as\s+buyer|counterparty|Principal\s+Capital|Commitment|Notes?)\b/i.test(text)) {
        return 'purchaser';
    }
    if (/\b(?:as\s+)?issuer|issuing\s+the\s+securities|issuable\s+hereunder|issuable\s+under|registrant|the\s+Company[\s,]+as\s+issuer/i.test(text)) {
        return 'issuer';
    }
 ...
 */
function buildLegalSignals(text, agr, rawCp, br) {
    let counterparty;
    ...
    const o = {};
    if (agr)
        o.agreement_type = agr;
    if (counterparty)
        o.counterparty = counterparty;
    if (counterparty_raw)
        o.counterparty_raw = counterparty_raw;
    if (role)
        o.role = role;
...
export function extractLayer1Signals(text) {
    ...
    if (exchange)
        signals.market_signals = { exchange };
    const sec = extractSecurityTypeLiteral(text);
    if (sec)
        signals.security_signals = { security_type: sec };
...
export function pruneLayer1Signals(s) {
    ...
    if (s.legal_signals) {
        ...
        if (s.legal_signals.counterparty)
            o.counterparty = s.legal_signals.counterparty;
        if (s.legal_signals.counterparty_raw) {
            o.counterparty_raw = s.legal_signals.counterparty_raw;
        }
        if (s.legal_signals.role)
            o.role = s.legal_signals.role;
        if (Object.keys(o).length)
            out.legal_signals = o;
    }
    return Object.keys(out).length > 0 ? out : undefined;
}
//# sourceMappingURL=explicit-signals.js.map