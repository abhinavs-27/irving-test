...
function dedupeEdges(relationships) {
    const seen = new Set();
    const out = [];
 ...
 */
export function finalizePricingInBlockRegistry(block_registry, sections) {
    ...
    for (const [bid, br] of Object.entries(block_registry)) {
        if (!br.pricing_model)
            continue;
        const paras = br.paragraph_ids.map((id) => paraMap.get(id)).filter(Boolean);
        ...
        const ok = modes.length > 0 &&
            modes.every((m) => m.discount_percent != null &&
        ...
        if (ok) {
            ...
            block_registry[bid] = rest;
        }
    }
}
export function enrichTerminationEvents(events, sections) {
    const paraMap = new Map();
    const walk = (nodes) => {
        for (const n of nodes) {
            if (n.type === 'paragraph')
...
export function ensureParagraphAtomicKinds(sections) {
...
export function augmentConstraintParagraphFacts(sections) {
    for (const s of sections) {
        ...
        for (const pid of cb.paragraph_ids) {
            ...
            if (!p?.facts)
                continue;
            const t = p.text;
            const pct = [...p.facts.percentages];
            const add = (n) => {
                if (!pct.some((x) => Math.abs(x - n) < 0.02))
                    pct.push(n);
            };
            if (/\b19\.99\b/.test(t))
                add(19.99);
...
export function ensureRelationshipCompleteness(payload, sections) {
    ...
    const cp = B_RILEY_PRINCIPAL_ID in entity_registry ? B_RILEY_PRINCIPAL_ID : undefined;
    for (const [bid, br] of Object.entries(block_registry)) {
        for (const pid of br.paragraph_ids) {
            const has = relationships.some((r) => r.type === 'governs' && r.source === bid && r.target === pid);
            if (!has) {
        ...
        if (br.semantic_role === 'termination') {
            ...
            if (ev) {
                const hasT = relationships.some((r) => r.type === 'triggers' && r.source === bid && r.target === ev.id);
                if (!hasT) {
                    relationships.push({
                        type: 'triggers',
                        source: bid,
    ...
    if (exec?.source_block_ids[0]) {
        const has = relationships.some((r) => r.type === 'triggers' &&
...
export function applyExtractionContractFixes(sections, payload) {
...
//# sourceMappingURL=extraction-contract-fix.js.map