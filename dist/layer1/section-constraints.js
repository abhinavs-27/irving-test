...
let seq = 0;
function nextId(sectionId, kind) {
    seq += 1;
    return `constraint:${sectionId}:${kind}:${seq}`;
}
function hasPctInText(t, n) {
    if (n === '4.99') {
 ...
 */
export function buildSectionConstraints(section) {
    if (section.type !== 'section' || !section.blocks?.length)
        return [];
    ...
    for (const b of section.blocks) {
        ...
        for (const pid of b.paragraph_ids) {
            ...
            const has1999 = hasPctInText(t, '19.99') ||
                pcts.some((x) => Math.abs(x - 19.99) < 0.01);
            const has499 = hasPctInText(t, '4.99') ||
            ...
            if (has1999) {
                out.push({
                    ...
                    description: 'Stated 19.99% cap (exchange / issuance), as in the paragraph',
                    values: {
            ...
            if (has499) {
                out.push({
                    ...
                    source_paragraph_ids: [pid],
                    source_block_id: b.id,
                    description: 'Stated 4.99% cap (beneficial / aggregate ownership), as in the paragraph',
...
//# sourceMappingURL=section-constraints.js.map