function tierAllowsSubtype(tier) {
    return tier === 'high';
}
export function derivePrimaryIntent(clauseType, text, tier) {
    const t = text.replace(/\s+/g, ' ');
    const allowSubtype = tierAllowsSubtype(tier);
    if (clauseType === 'misc.noise')
        return 'unclassified';
    const base = clauseType;
    switch (base) {
        case 'pricing_terms': {
            if (!allowSubtype)
                return 'pricing_terms.general';
            const hasVwap = /\bVWAP\b/i.test(t);
            const hasDiscount = /\bdiscount\b/i.test(t) ||
                /\b\d+(?:\.\d+)?\s*%\s*(?:discount|below)/i.test(t);
            const hasFixedCap = /\bcap\b/i.test(t) && /\$\d/.test(t);
            if (hasVwap && hasDiscount)
                return 'pricing_terms.vwap_discount';
            if (hasVwap)
                return 'pricing_terms.vwap';
            if (hasDiscount && !hasVwap)
                return 'pricing_terms.fixed_discount';
            if (hasFixedCap)
                return 'pricing_terms.price_cap';
            return 'pricing_terms.general';
        }
        case 'payment':
        ...
        case 'obligations':
            ...
            if (/\bdeliver(?:y|ies)?\b/i.test(t))
                return 'obligations.delivery';
        ...
        case 'termination':
            ...
            if (/\bbankruptcy\b/i.test(t))
                return 'termination.bankruptcy';
            if (/\bEvent of Default\b/i.test(t))
...
//# sourceMappingURL=primary-intent.js.map