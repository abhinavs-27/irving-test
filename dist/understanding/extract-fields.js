import { extractIsoDates, extractPercentValues, extractUsdAmounts, parsePercentLiteral, parseUsdScalar, } from './normalize-values.js';
function extractTermination(text) {
    const notice = text.match(/\b(\d{1,3})\s*(?:calendar\s+)?(?:business\s+)?days?\s+(?:'|’|')?(?:advance\s+)?notice\b/i) ?? text.match(/\b(\d{1,3})\s*(?:business\s+)?days?\b/i);
    ...
    if (/\b(?:material|MAC)\s+breach\b/i.test(text))
        triggers.push('material_breach');
    ...
    const mutual_termination_allowed = /\bmutual\b/i.test(text) && /\bterminat/i.test(text)
        ? true
    ...
    return {
        ...
        references_automatic_termination: /\bautomatic(?:ally)?\s+terminat/i.test(text)
            ? true
            : null,
        material_dates_iso: extractIsoDates(text).length > 0 ? extractIsoDates(text) : null,
    };
}
/** Semantic pricing: models, discount rows bound to VWAP/trading context, caps. */
function extractPricingTerms(text) {
    let pricing_model = null;
    const hasVwap = /\bVWAP\b/i.test(text);
    if (hasVwap)
        pricing_model = 'volume_weighted_average_price';
    else if (/\bfixed\s+price\b/i.test(text))
        pricing_model = 'fixed_price';
    else if (/\bper\s+share\b/i.test(text))
        pricing_model = 'per_share';
    const pct = parsePercentLiteral(text);
    const hasDiscountCue = /\bdiscount\b/i.test(text) ||
        /\b\d+(?:\.\d+)?\s*%\s*(?:below|discount|off)/i.test(text);
    const tradingSession = /\bregular\s+trading\s+(?:session|hours)\b/i.test(text) ||
        /\b(?:market|trading)\s+session\b/i.test(text);
    const tradingDaysMention = /\b(?:trading|intraday)\s+days?\b/i.test(text) ||
        /\bfive\s+trading\s+days\b/i.test(text);
    const discount_terms = [];
    if (pct != null || (hasDiscountCue && hasVwap)) {
        let applies_to = null;
        if (hasVwap)
            applies_to = 'volume_weighted_average_price';
        else if (/\bpurchase\s+price\b/i.test(text))
            applies_to = 'purchase_price';
        let condition = null;
        if (tradingSession)
            condition = 'regular_trading_session';
        else if (tradingDaysMention)
            condition = 'trading_days_window';
        else if (hasVwap || pct != null)
    ...
    let cap_usd = null;
    const capM = text.match(/\b(?:cap|ceiling|limit)(?:ed|s)?\s+(?:of|at)?\s*\$\s*([\d,.]+)/i);
    if (capM)
        cap_usd = parseUsdScalar(capM[1] ?? capM[0]);
    const WORD_NUM = {
        one: 1,
        two: 2,
        three: 3,
    ...
    const wm = text.match(/\b(one|two|three|four|five|six|seven|eight|nine|ten)\s+trading\s+days?\b/i);
    let trading_days_window = wm
...
function extractConstraints(text) {
    const regulatory_threshold_percents = extractPercentValues(text).length > 0
        ? extractPercentValues(text)
        : null;
    const ownership_cap_percent = /\b(?:ownership|beneficial|voting)\b/i.test(text)
        ? parsePercentLiteral(text)
        : null;
    return {
        regulatory_threshold_percents,
        ownership_cap_percent,
        issuance_restriction_referenced: /\bissuance\s+(?:limit|cap|restriction)\b/i.test(text)
            ? true
            : null,
...
function extractConfidentiality(text) {
    const scope_codes = [];
    if (/\bconfidential\s+information\b/i.test(text))
        scope_codes.push('confidential_information');
    if (/\bproprietary\b/i.test(text))
        scope_codes.push('proprietary');
    const dm = text.match(/\b(?:for|after)\s+(?:a\s+period\s+of\s+)?(\d+)\s*(year|month)s?\b/i);
    let duration_months = null;
    if (dm?.[2]) {
        const n = Number.parseInt(dm[1], 10);
        if (dm[2].toLowerCase().startsWith('year'))
...
function extractIndemnity(text) {
    let indemnity_scope_code = null;
    if (/\bthird[\s-]?party\s+claims?\b/i.test(text))
        indemnity_scope_code = 'third_party_claims';
    else if (/\bindemnif/i.test(text))
        indemnity_scope_code = 'general';
    const surv = text.match(/\bsurviv(?:e|al)\s+(?:for|until)(?:\s+a\s+period\s+of)?\s+(\d+)\s*(year|month)s?\b/i);
    let survival_months = null;
    let survival_years = null;
    if (surv?.[2]) {
        const n = Number.parseInt(surv[1], 10);
        if (surv[2].toLowerCase().startsWith('year'))
            survival_years = n;
        else
            survival_months = n;
    }
    const indemnitor_role_codes = [];
    if (/\bCompany\b/.test(text))
        indemnitor_role_codes.push('company');
    if (/\bSeller\b/.test(text))
        indemnitor_role_codes.push('seller');
    if (/\bParent\b/.test(text))
        indemnitor_role_codes.push('parent');
    let basket_threshold_usd = null;
    const basketM = text.match(/\bbasket\b[^.$]{0,120}?\$\s*([\d,.]+)/i);
    if (basketM)
        basket_threshold_usd = parseUsdScalar(basketM[1]);
    else {
        const nearBasket = /\bbasket\b/i.test(text);
        if (nearBasket) {
            const first = extractUsdAmounts(text);
            if (first.length > 0)
                basket_threshold_usd = first[0];
        }
    }
    return {
        ...
        indemnitor_role_codes: indemnitor_role_codes.length > 0 ? indemnitor_role_codes : null,
    };
}
function extractObligations(text) {
    ...
    const performance_due_iso = dates.length === 1 ? dates[0] : dates.length > 1 ? dates[0] : null;
    return {
        shall_count: shall_count > 0 ? shall_count : null,
        must_count: must_count > 0 ? must_count : null,
        within_days_deadlines: deadlines.length > 0 ? [...new Set(deadlines)] : null,
        performance_due_iso,
...
export function extractFieldsForSubstantiveType(t, text) {
    switch (t) {
        case 'termination':
