import { isMaterialDefinitiveItemSection, isStructuredItemSection, parseSectionIdFromClauseId, sectionSuggestsClauseTypes, } from './section-context.js';
/**
 * Field-fill ratio: non-null meaningful values / total keys.
 */
export function computeFieldFillRatio(fields) {
...
export function blendConfidence(fieldRatio, separationStrength) {
...
export function lengthPrior(text) {
...
export function computeUnderstandingConfidence(substantiveType, fullFields, separationStrength, text) {
...
export function calibrateConfidence(base, opts) {
    let s = base;
    const sectionId = parseSectionIdFromClauseId(opts.clauseId);
    const hints = sectionSuggestsClauseTypes(sectionId);
    if (opts.clauseType !== 'misc.noise' &&
        hints.some((h) => h === opts.clauseType)) {
        s += 0.06;
    }
    if (opts.entitiesResolved)
        s += 0.05;
    if (opts.fragmentedClause)
        s -= 0.06;
    if (opts.noiseLikelyMisclassified)
        s -= 0.12;
    if (opts.missingEntitiesForSignificantClause)
        s -= 0.07;
    return Math.round(Math.min(1, Math.max(0, s)) * 100) / 100;
}
export function inferFragmentation(text) {
    const t = text.replace(/\s+/g, ' ').trim();
    return t.length > 0 && t.length < 35 && !/\.$/.test(t);
}
export function entitiesAreResolved(parties, instruments) {
    const p = Boolean(parties.company || parties.counterparty);
    const i = Boolean(instruments && instruments.length > 0);
    return p || i;
}
export function shouldFlagMissingEntitiesForSection(clauseType, clauseId, entitiesResolved) {
    if (entitiesResolved)
        return false;
    const sid = parseSectionIdFromClauseId(clauseId);
    if (!isMaterialDefinitiveItemSection(sid))
        return false;
    return (clauseType === 'pricing_terms' ||
        clauseType === 'termination' ||
        clauseType === 'payment');
}
export function noiseMisclassifiedSuspicion(clauseType, clauseId, hadSemanticSignals) {
    return (clauseType === 'misc.noise' &&
