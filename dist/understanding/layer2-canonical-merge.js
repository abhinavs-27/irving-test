...
function mergeTerminationUnion(blocks) {
    const m = {};
    let agg;
    let months;
    let days;
    ...
    if (agg !== undefined)
        m.aggregate_purchase_ceiling_usd = agg;
    if (months !== undefined)
        m.stated_term_months = months;
    if (days !== undefined)
        m.stated_term_days = days;
    if (notice !== undefined)
        m.termination_notice_days = notice;
    return m;
}
/**
 ...
 */
function terminationPayloadNonEmpty(c) {
    const t = c.extracted_fields.termination;
    return !!t && Object.keys(t).length > 0;
}
export function mergeTerminationDomainIntoCanonical(clauses) {
    const terminationRows = clauses.filter((c) => c.clause_type === 'termination' && terminationPayloadNonEmpty(c));
    if (terminationRows.length <= 1)
        return clauses.map(cloneBlock);
...
//# sourceMappingURL=layer2-canonical-merge.js.map