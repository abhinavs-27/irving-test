...
export function percentPointsToRate(points) {
 ...
 */
export function terminationFlatFromFacts(facts) {
    ...
    const out = {};
    if (monthW.length > 0)
        out.stated_term_months = monthW[0].value;
    else if (yearW.length > 0)
        out.stated_term_months = yearW[0].value * 12;
    ...
    if (out.stated_term_months !== undefined &&
        thirtyCount === 1 &&
...
export function timeWindowsToExplicit(tws) {
    let months;
    let days;
    let years;
    for (const tw of tws) {
        if (tw.unit === 'month' && months === undefined)
            months = tw.value;
        else if (tw.unit === 'day' && days === undefined)
            days = tw.value;
        else if (tw.unit === 'year' && years === undefined)
            years = tw.value;
    }
    return {
...
export function isNonEmptyFacts(f) {
    return (f.percentages.length > 0 ||
...
export function maxUsd(facts) {
    const vals = facts.dollar_amounts.map((d) => d.value);
    if (vals.length === 0)
        return undefined;
    return Math.max(...vals);
}
export function minPriceThresholdUsd(facts) {
    const vals = facts.price_thresholds.map((d) => d.value);
    if (vals.length === 0)
        return undefined;
    return Math.min(...vals);
}
export function maxShareCount(facts) {
    if (facts.share_counts.length === 0)
        return undefined;
    return Math.max(...facts.share_counts);
}
export function dateEarliestLatest(facts) {
...
export function statedTermMonthsFromFacts(facts) {
    const tw = facts.time_windows;
    const monthW = tw.filter((t) => t.unit === 'month');
    const yearW = tw.filter((t) => t.unit === 'year');
    if (monthW.length > 0)
        return monthW[0].value;
    if (yearW.length > 0)
        return yearW[0].value * 12;
    return undefined;
}
//# sourceMappingURL=layer2-normalize.js.map