...
function layer1BlockTypeToClauseType(t) {
    switch (t) {
...
function applyStructuralConfidenceCap(clauseType, confidence, linkedEventsCount, extracted) {
    if (clauseType !== 'structural')
        return confidence;
    const s = extracted.structural;
    const hasBothDates = !!s?.agreement_reference_date_iso &&
...
function collectParagraphMap(roots) {
    const m = new Map();
    const walk = (n) => {
        if (n.type === 'paragraph')
            m.set(n.id, n);
...
function collectAllSectionConstraints(roots) {
    const out = [];
    const walk = (n) => {
        if (n.type === 'section') {
            if (n.constraints?.length) {
                ...
                out.push(...buildSectionConstraints(n));
            }
        }
        for (const c of n.children ?? [])
            walk(c);
...
function constraintsForBlock(block, all) {
    const pset = new Set(block.paragraph_ids);
    return all.filter((c) => c.source_block_id === block.id ||
        c.source_paragraph_ids.some((pid) => pset.has(pid)));
}
function sliceRelationships(blockId, relationships) {
    const governs = [];
    const constrains = [];
    const references = [];
...
function linkedEventsForBlock(blockId, events, relationships) {
    const byId = new Map(events.map((e) => [e.id, e]));
    const out = new Map();
    for (const e of events) {
        if (e.source_block_ids.includes(blockId))
            out.set(e.id, e);
    }
    for (const r of relationships) {
        if (r.source === blockId &&
...
function paragraphCounterpartyHints(block, paraById) {
    const ids = new Set();
    for (const pid of block.paragraph_ids) {
        const p = paraById.get(pid);
        const cid = p?.signals?.legal_signals?.counterparty_id;
        if (cid && cid.startsWith(ENTITY_ID_PREFIX))
            ids.add(cid);
    }
    return [...ids].sort();
}
function resolveClauseEntities(blockId, linked, relationships, issuerId, paraCpIds) {
    if (linked.length > 0) {
        ...
        for (const e of linked) {
            for (const x of e.counterparty_entity_ids)
                cp.add(x);
        }
    ...
    const orgFromEdges = new Set();
    for (const r of relationships) {
        if (r.source !== blockId)
            continue;
        if ((r.type === 'constrains' || r.type === 'defines') &&
...
function structuredConfidence(clauseType, extracted, linkedEventsCount) {
    let score = clauseType === 'structural' || clauseType === 'other'
        ? 0.42
...
export function buildLayer1FilingInput(input) {
    return {
...
export function projectNormalizedClausesFromLayer1(filing) {
    ...
    const issuerId = sections.length > 0
        ? resolveIssuerEntityId(sections, filing.entity_registry)
        : undefined;
    const blockEntries = Object.values(filing.block_registry).sort((a, b) => a.id.localeCompare(b.id));
    const raw = [];
    for (const block of blockEntries) {
        ...
        for (const pid of block.paragraph_ids) {
            const p = paraById.get(pid);
            if (p?.facts)
                factsParts.push(p.facts);
        }
        const merged = factsParts.length > 0 ? mergeFacts(factsParts) : undefined;
        const relevantConstraints = constraintsForBlock(block, allConstraints);
        const linked = linkedEventsForBlock(block.id, filing.events, filing.relationships);
        const paraHints = paragraphCounterpartyHints(block, paraById);
        const entities = resolveClauseEntities(block.id, linked, filing.relationships, issuerId, paraHints);
        const extracted = buildExtractedFieldsForBlock(clause_type, block, merged, relevantConstraints, paraById, linked, entities.primary_entity_id ?? '', entities.counterparty_entity_ids) ?? {};
        ...
        const rec = {
            ...
            confidence: applyStructuralConfidenceCap(clause_type, baseConfidence, linked.length, extracted),
...
//# sourceMappingURL=layer2-from-layer1.js.map