import { dateEarliestLatest, isNonEmptyFacts, maxShareCount, maxUsd, percentPointsToRate, terminationFlatFromFacts, usdToNumber, } from './layer2-normalize.js';
function emptyFacts() {
    return {
        percentages: [],
        dollar_amounts: [],
...
function rawModeFromLayer1(m) {
    const vwap_session = m.vwap_window === 'intraday_segments'
        ? 'intraday_segments'
...
function inferMechanismAndSettlement(block, pm, paraById) {
    ...
    for (const pid of block.paragraph_ids) {
        ...
        if (m === 'VWAP')
            return { mechanism: 'vwap_discount', settlement_method: 'VWAP' };
        if (m === 'FIXED')
            return { mechanism: 'fixed_price', settlement_method: 'FIXED' };
        if (m === 'FORMULA')
...
export function buildPricingSchema(block, merged, pm, paraById) {
    const { mechanism, settlement_method } = inferMechanismAndSettlement(block, pm, paraById);
    const rawModes = mechanism === 'vwap_discount' && pm ? pm.modes.map(rawModeFromLayer1) : [];
...
function exchangeFromSection(row) {
    const vals = row.values;
    const out = {};
    const other = [];
    if (vals?.share_cap !== undefined)
        out.share_cap = vals.share_cap;
    if (vals?.percentages?.length) {
        out.issuance_cap_rate = percentPointsToRate(Math.max(...vals.percentages.map(Math.abs)));
    }
    if (vals?.dollar_cap) {
        other.push({
            kind: 'usd_ceiling',
...
function beneficialFromSection(row) {
    const vals = row.values;
    if (!vals?.percentages?.length)
        return null;
    return {
...
export function buildConstraintSchema(rows, merged, issuerId, counterpartyIds) {
    const exchange_issuance = [];
    ...
    if (rows.length === 0 && facts && isNonEmptyFacts(facts)) {
        ...
        if (appliesIssuer && (maxPct !== undefined || maxShares !== undefined)) {
            ...
            if (maxShares !== undefined)
                r.share_cap = maxShares;
            if (maxPct !== undefined)
                r.issuance_cap_rate = percentPointsToRate(maxPct);
    ...
    const c = {};
    if (exchange_issuance.length > 0)
        c.exchange_issuance = exchange_issuance;
    if (beneficial_ownership.length > 0)
        c.beneficial_ownership = beneficial_ownership;
    if (other_accum.length > 0)
        c.other_constraints = other_accum;
    return c;
}
export function buildTerminationSchema(merged) {
    const facts = merged ?? emptyFacts();
    return terminationFlatFromFacts(facts);
}
export function buildDisclosureSchema(merged) {
    const facts = merged ?? emptyFacts();
...
export function buildStructuralSchema(merged, executionDateIso) {
    const facts = merged ?? emptyFacts();
...
function executionDateFromEvents(linked) {
    const exec = linked.find((e) => e.kind === 'agreement_execution');
    return exec?.as_of_date;
}
export function buildExtractedFieldsForBlock(clause_type, block, mergedFacts, relevantConstraints, paraById, linkedEvents, issuerId, counterpartyEntityIds) {
    const out = {};
    const execIso = executionDateFromEvents(linkedEvents);
    switch (clause_type) {
        case 'pricing_terms':
            out.pricing = buildPricingSchema(block, mergedFacts, block.pricing_model, paraById);
            break;
        case 'constraint':
            out.constraints = buildConstraintSchema(relevantConstraints, mergedFacts, issuerId, counterpartyEntityIds);
            break;
        case 'termination':
...
export function pruneEmptyExtractedFields(ef) {
    const o = {};
...
//# sourceMappingURL=layer2-extracted-build.js.map